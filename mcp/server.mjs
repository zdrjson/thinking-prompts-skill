...
/* 【占位符】→ 参数名 slot1..slotN */
const slotsOf = p => [...p.body.matchAll(/【([^】]*)】/g)].map((m, i) => ({
  name: `slot${i + 1}`, description: m[1], required: false,
}));

function fill(p, args = {}) {
  let i = 0;
  return p.body.replace(/【([^】]*)】/g, (_, ph) => {
    const v = args[`slot${++i}`];
    return v && String(v).trim() ? String(v) : `【${ph}】`;
  });
}

const index = () => D.prompts
  .map(p => `${String(p.n).padStart(2, '0')}  ${p.id.padEnd(20)} ${p.name} —— ${p.when}`)
  .join('\n');
...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string', enum: D.prompts.map(p => p.id), description: '框架 id' },
      },
      required: ['id'],
    },
  },
  ...
  switch (method) {
    case 'initialize':
      return ok(id, {
        ...
        instructions:
          `12 个思考框架（${D.source.author}）。分流：${D.routing}\n${RULES}`,
      });
    ...
    case 'tools/call': {
      ...
      if (name === 'list_thinking_frameworks')
        return ok(id, txt(`12 个思考框架：\n\n${index()}\n\n分流：${D.routing}`));
      if (name === 'get_thinking_framework') {
        const p = BY_ID[a.id];
        if (!p) return ok(id, txt(`没有 id 为 "${a.id}" 的框架。可用：\n\n${index()}`));
        return ok(id, txt(
          `# ${p.name}（${p.groupName}）\n\n什么时候用：${p.when}\n\n## Prompt 正文\n\n${p.body}\n\n`
          + `## 要点\n\n${p.note}\n\n## ${RULES}`));
      }
    ...
    case 'prompts/list':
      return ok(id, {
        prompts: D.prompts.map(p => ({
          name: p.id,
          title: `${p.n}. ${p.name}`,
          description: `${p.when} —— ${p.groupName}`,
          arguments: slotsOf(p),
        })),
      });
    ...
    case 'prompts/get': {
      ...
      if (!p) return err(id, -32602, `未知框架：${params.name}`);
      return ok(id, {
        description: `${p.name} —— ${p.when}`,
        messages: [{ role: 'user', content: { type: 'text', text: fill(p, params.arguments) } }],
      });
    }
